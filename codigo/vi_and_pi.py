...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for s in range(nS):
        ...
        for a in range(nA):
            q_values[a] = sum(prob * (reward + gamma * value_from_policy[next_state])
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while True:
        ...
        for s in range(nS):
            v = value_function[s]
            q_values = [sum(prob * (reward + gamma * value_function[next_state])
                            for prob, next_state, reward, _ in P[s][a])
                        for a in range(nA)]
